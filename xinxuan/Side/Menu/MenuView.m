...
+(instancetype)MenuViewWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    ...
    MenuView *menu = [[MenuView alloc]initWithDependencyView:dependencyView MenuView:leftmenuView isShowCoverView:isCover];//第一个参数为页面view 第二个参数为 左侧的view 第三个参数为YES
...
-(instancetype)initWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    //第一个参数为页面view 第二个参数为 左侧的view 第三个参数为YES

    if(self = [super init]){
        self.isShowCoverView = isCover;
        [self initView];//根据bool属性判断颜色
        [self setDependencyView:dependencyView];
        self.leftMenuView = leftmenuView;//根据传过来view给左侧的view赋值
        if((CGRectEqualToRect(_coverViewframe, CGRectZero)) == NO){
            self.menuViewframe = leftmenuView.frame;
        }else{
        }
        self.coverViewframe = CGRectMake(self.menuViewframe.size.width, self.menuViewframe.origin.y, kSCREEN_WIDTH - self.menuViewframe.size.width, self.menuViewframe.size.height);//当前页面的frame为window界面－左侧的frame
    }
    return self;
}

#pragma mark--给当前页面绑定屏幕边缘手势
...
-(void)initView{
    ...
    if(self.isShowCoverView)//被覆盖的情况 ＝》左侧出现
    {
//        self.coverView.backgroundColor = CoverViewBackGround;
         self.coverView.backgroundColor = [UIColor cyanColor];
    }else{
        self.coverView.backgroundColor = [UIColor orangeColor];
    }
...
#pragma mark---点击按钮可显示左侧view
-(void)show{
    
    UIWindow * window = [[UIApplication sharedApplication].delegate window];
    [window addSubview:self.coverView];
    [window addSubview:self.leftMenuView];//此时已经有值
